/* WorkoutPlayer · punto d'ingresso della libreria (API v0: lib/API-v0.md; API v1: lib/API-v1.md).
   ...
   lang         lingua dell'interfaccia fra it, en, fr, es, de (lib/i18n.js); senza, la lingua del piano se è una delle cinque,
                altrimenti italiano (T2 passo 9, #61: il player parla e scrive nella lingua della scheda; l'app può chiedere la sua).
                La voce usa sempre la lingua del piano.
...
(function (WP) {
  ...
  // lingua dell'interfaccia del player: quella chiesta (opzione lang), altrimenti quella del piano se è una delle cinque,
  // altrimenti italiano (le pagine di build_plan.py senza --lang: il piano del titolare, in italiano, resta com'era)
  const uiLang = (plan, lang) => WP.i18n.code(lang) || WP.i18n.code(plan.lang) || "it";
  ...
  WP.create = function (opts) {
    ...
    const lang = uiLang(plan, opts.lang);
    const R = {
  ...
  WP.start = async function (opts) {
    ...
    if (opts.composeSource) {
      const res = await WP.composeIsolated(opts.plan, opts.composeSource, opts.timeoutMs, uiLang(opts.plan, opts.lang));
