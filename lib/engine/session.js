/* WorkoutPlayer · motore: stato della seduta, timer su orari, pausa e ritocchi, persistenza, annunci, fine e uscita.
   ...
   - carico "−/+" (passo 9, decisione del titolare #56): il ritocco è la differenza dal carico in scheda, per esercizio (S.kgDiff),
     e vale sopra il carico in scheda di ogni voce di quell'esercizio (settimane, altre sedute), mai sotto zero (changeKg);
...
(function (WP) {
  ...
  WP.modules.session = function (R) {
    ...
    // "−/+" sul carico (T2 passo 9, decisione del titolare #56): il ritocco è la differenza dal carico in scheda (S.kgDiff), calcolata
    // sulla voce del riquadro toccato (il passo in corso, o "Tra poco" nel recupero) con il passo dell'attrezzo (plan.kgStep, 1 kg);
    // vale per quell'esercizio in ogni voce, sopra il suo carico in scheda, mai sotto zero
    function changeKg(exId, dir) {
      const mine = i => i && i.ex === exId && typeof i.kg === "number";
      const item = [R.STEPS[R.cur.idx], nextWork(R.cur.idx)].map(s => (s && s.ii != null ? itemOf(s) : null)).find(mine) || R.W.phases.flatMap(p => p.items).find(mine);
      if (!item) return;
      const shown = Math.max(0, Math.round((R.kgOf(item) + dir * ((R.plan.kgStep || {})[exId] || 1)) * 100) / 100), diff = Math.round((shown - item.kg) * 100) / 100;
      if (diff) S.kgDiff[exId] = diff; else delete S.kgDiff[exId];
      store.save(); R.render();
    }
    function logStep(i) {
    ...
    return { phaseOf, itemOf, nextWork, blockItems, estimateMinutes, countSets, persist, resumable, locate, sessionSteps, startWorkout, resumeInterrupted, enter, announce, addRound, remainingMs, tick, togglePause, adjust, finish, leave,
      canRemoveSet, editSets, repsDone, changeReps, changeKg };
  };
