...
(function (WP) {
  ...
  /* Esegue lo script isolato. Risolve sempre (mai rifiuta): { ok: true, steps } oppure { ok: false, error }. lang: lingua
     dell'interfaccia per il titolo (nascosto) del riquadro.
     timeoutMs vale per l'esecuzione nel Worker; l'ospite aspetta al massimo timeoutMs + 3 s anche se il riquadro non risponde. */
  function composeIsolated(plan, composeSource, timeoutMs, lang) {
    timeoutMs = timeoutMs || 2000;
    return new Promise(resolve => {
      const frame = document.createElement("iframe");
      frame.setAttribute("sandbox", "allow-scripts");
      frame.hidden = true; frame.title = WP.i18n.t(lang || "it", "aria.scriptFrame");
