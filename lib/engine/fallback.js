  ...
  function fallbackV1(w) {
    ...
    w.phases.forEach((ph, pi) => {
      ...
      if (flow === "sets" || flow === "continuous") {
        ...
        // una serie per voce a ogni giro; più serie per giro solo se i dati le hanno (T2: serie corretta dall'utente in revisione)
        for (let r = 1; r <= ph.rounds; r++) ph.items.forEach((it, ii) => { const n = posInt(it.sets) ? it.sets : 1; for (let set = 1; set <= n; set++) emitSet(ii, set, n, r); });
