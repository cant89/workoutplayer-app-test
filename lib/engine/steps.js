...
(function (WP) {
  ...
  // durata di una voce a tempo: secondi, oppure (T2) un intervallo { min, max } dato dall'utente in revisione
  const durOk = d => posNum(d) || (!!d && typeof d === "object" && posNum(d.min) && posNum(d.max) && d.max > d.min);
  ...
  function guidedV1(ph, it) {
    ...
    if (it.mode === "timed") return durOk(it.dur) && !it.parts && !it.drops && it.hold == null;
  ...
  /* Durata e orologio di un passo: { secs, view, min }. secs > 0 = conto alla rovescia di secs; 0 = cronometro in avanti;
     min (T2, durata a intervallo): secondi dopo i quali la durata minima è raggiunta.
  ...
  function timing(version, w, st, prepSec) {
    ...
    // durata a intervallo: conto alla rovescia sul massimo, con il minimo segnato (min: bip e avviso quando è raggiunto)
    if (it.mode === "timed") return typeof it.dur === "object" ? { secs: it.dur.max, view: "down", min: it.dur.min } : { secs: it.dur, view: "down" };
  ...
  WP.timing = timing;
  /* T2 passo 9 — mai un numero perduto in silenzio. La sequenza (script o riserva: lib/API-v1.md, "nessun recupero prima di una
     nuova fase") non fa il recupero dopo l'ultima serie di una fase: la fase dopo comincia dal suo ingresso, dove ci si ferma
     comunque. Quando però quel recupero è l'unico punto in cui i dati danno quel valore (una voce di una sola serie a fine fase, la
     voce "Recupero fra blocchi" di una fase a sé, il recupero dell'ultimo giro diverso dagli altri, una fase a un solo giro), il
     player lo fa: un passo di recupero prima della fase dopo (keepRests). Dopo l'ultima fase della seduta non c'è una fase dopo: il
     recupero lo scrive la schermata finale (endRest). Solo piani v1 (lib/index.js): i piani v0 restano quelli di T0. */
  const restValue = (ph, s) => restSecondsV1(ph, s.ii, s.round || 1) || "testo: " + (ph.items[s.ii].restText || "");
  function lostRestV1(w, steps, pi) {
    let at = -1;
    steps.forEach((s, i) => { if (s.pi === pi) at = i; });
    const last = steps[at], ph = w.phases[pi];
    if (!last || !ph || last.type === "rest" || last.type === "gate" || last.ii == null) return null;
    const it = ph.items[last.ii];
    if (!it || !hasRestV1(ph, last.ii, last.round || 1) || (it.noRestAfterLast && last.set === last.sets)) return null;
    const v = restValue(ph, last), shared = INTERVAL_FLOWS.includes(ph.flow); // negli intervalli il recupero è della fase
    if (steps.some(s => s.pi === pi && s.type === "rest" && (shared || s.ii === last.ii) && restValue(ph, s) === v)) return null;
    return { at, step: { type: "rest", pi, ii: last.ii, set: last.set, sets: last.sets, round: last.round } };
  }
  function keepRestsV1(w, steps) {
    if (!Array.isArray(steps)) return steps;
    let out = steps;
    w.phases.forEach((ph, pi) => {
      const lost = lostRestV1(w, out, pi);
      if (lost && lost.at < out.length - 1) out = out.slice(0, lost.at + 1).concat([lost.step], out.slice(lost.at + 1));
    });
    return out;
  }
  // il recupero perso dopo l'ultimo passo della seduta: { type: "rest", pi, ii, … } oppure null
  function endRestV1(w, steps) {
    if (!Array.isArray(steps) || !steps.length) return null;
    const lost = lostRestV1(w, steps, steps[steps.length - 1].pi);
    return lost && lost.at === steps.length - 1 ? lost.step : null;
  }

  WP.v1 = { durOk, BLOCK_FLOWS, INTERVAL_FLOWS, ROUND_FLOWS, repsOk, guided: guidedV1, blockOk, intervalsOk, unitsOf, unitTimed, restSeconds: restSecondsV1, hasRest: hasRestV1, holdAt, holdSeconds,
    keepRests: keepRestsV1, endRest: endRestV1 };
