...
(function (WP) {
  ...
  WP.modules.texts = function (R) {
    ...
    // carico della voce: in scheda + ritocco dell'esercizio (differenza, S.kgDiff, lib/engine/store.js), mai sotto zero
    const kgOf = item => (typeof item.kg === "number" ? Math.max(0, Math.round((item.kg + (S.kgDiff[item.ex] || 0)) * 100) / 100) : item.kg);
    ...
    const rawSummary = (it, lang) => [
      ...
      it.dur != null ? (typeof it.dur === "number" || typeof it.dur === "object" ? durText(it.dur) : String(it.dur)) : "",
    ...
    // serie, giro, blocco o intervallo di un passo: { kind, n, of, side, set } (kind vuoto = nessuna etichetta); set = serie dentro
    // il giro di un circuito, solo se la voce ne ha più di una per giro (T2 passo 9: serie per giro corretta in revisione)
    const setInfo = st => {
      ...
      if (ph.flow === "circuit") return { kind: "round", n: st.round, of: ph.rounds, side: R.v1 && it && it.perSide && st.side != null ? it.sides[st.side] : null, set: R.v1 && st.sets > 1 ? { n: st.set, of: st.sets } : null };
    ...
    const setLabel = (st, lang) => { const i = setInfo(st); return i.kind ? T(lang, "label." + i.kind, i) + (i.set ? " · " + T(lang, "label.set", i.set) : "") + (i.side ? " · " + i.side : "") : ""; };
