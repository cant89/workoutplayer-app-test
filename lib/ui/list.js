  ...
  WP.modules.list = function (R) {
    ...
    function renderList() {
      ...
      $("listBody").innerHTML = R.W.phases.map((ph, pi) => '<div class="sl-phase">' + esc(ph.title) + "</div>" + ph.items.map((it, ii) => {
        ...
        const sum = R.itemSummary(it, ph), long = R.v1 && sum.length > 22 ? " long" : ""; // v1: un riepilogo lungo va a capo (piani v0 come l'artifact)
        return '<button type="button" class="sl-item' + (now ? " now" : done ? " done" : "") + long + '" data-jump="' + target + '"><span>' + esc(EX[it.ex].name) + "</span><span>" + esc(sum) + "</span></button>";
      }).join("")).join("");
    }
    return { renderList };
  };
