/* WorkoutPlayer · componente: il player (barra in alto, avanzamento per fase, disegno del passo corrente).
   ...
   Durata a intervallo (T2, dalla revisione): sotto il quadrante "almeno N" fino al minimo, poi "puoi fermarti".
   T2 passo 9 (#61): se il telefono non ha una voce nella lingua della scheda, un avviso nel passo in cui la prima frase non si è
   potuta dire, una volta sola (showVoiceNotice, chiamato da lib/engine/audio.js). */
(function (WP) {
  "use strict";
  ...
  WP.modules.player = function (R) {
    ...
    // "Serie 1 di 4 · Lato sinistro" → [Serie 1/4] [Lato sinistro]; "Giro 2 di 3 · Serie 1 di 2" → [Giro 2/3] [Serie 1/2];
    // info = R.setInfo(st) o R.partInfo(st)
    const infoChips = info => {
      if (!info || !info.kind) return "";
      return chip(T("chip." + info.kind), info.n + "/" + info.of) + (info.set ? chip(T("chip.set"), info.set.n + "/" + info.set.of) : "") +
        (info.side ? chip(T("chip.side"), R.sideValue(info.side)) : "");
    };
    function kgChipHTML(it) {
      if (typeof it.kg === "number") {
        const changed = R.kgOf(it) !== it.kg;
    ...
    function marksHTML(st, withItem) {
      ...
      const KEYS = { corrected: "mark.corrected", confirmed: "mark.confirmed", unverified: "mark.unverified" };
      return '<p class="gear">' + esc(list.map(m => T(KEYS[m.state] || KEYS.confirmed, { field: T("field." + m.field), value: m.value, original: m.original })).join(" · ")) + "</p>";
    }
    ...
    function render() {
      ...
      // T2 passo 9 (#61): il telefono non ha una voce nella lingua della scheda; si dice nel passo in cui è successo, una volta sola
      const voice = R.voiceNotice != null && R.voiceNotice === cur.stepAt ? '<div class="note" id="voiceNotice"><span class="eyebrow">' + T("audio.voice") + "</span>" +
        esc(T("audio.noVoice", { lang: R.planLangName() })) + "</div>" : "";
      $("stage").innerHTML = voice + fallback + away + h; $("btnMain").textContent = main;
    ...
    function paintClock(ms) {
      ...
      if (cur.rangeLeftMs != null && !cur.paused) { // durata a intervallo: prima del minimo "almeno 30 s", poi "puoi fermarti"
        const sub = document.querySelector("#stage .dial .sub"), it = R.itemOf(R.STEPS[cur.idx]);
        if (sub) sub.textContent = ms <= cur.rangeLeftMs ? T("dial.rangeDone") : T("dial.rangeMin", { t: durText(it.dur.min) });
      }
    ...
    // barra delle fasi; il player si apre (in cima) solo se non è già aperto: "+ Serie" e "− Serie" la ridisegnano restando dove si è
    function mountPlayer() {
      $("track").innerHTML = R.W.phases.map((ph, pi) => '<i style="flex:' + R.STEPS.filter(s => s.pi === pi).length + '"><b></b></i>').join("");
      if ($("player").hidden) R.show("player");
    }

    // chiamato dalla voce (lib/engine/audio.js) quando una frase non si può dire: avviso nel passo in corso
    function showVoiceNotice() { if (!R.cur) return; R.voiceNotice = R.cur.stepAt; R.render(); }

    return { render, paintAge, paintClock, paintElapsed, mountPlayer, showVoiceNotice };
  };
